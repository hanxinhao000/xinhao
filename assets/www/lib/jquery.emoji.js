    ...
    Plugin.prototype = {
        _init: function () {
            ...
            for (var i = 0; i < groupLength; i++) {
                ...
                panel += '</ul></div>';
                emoji_content += panel;
                emoji_tab += '<li data-emoji_tab="emoji' + i + '" class="' + (i === 0 ? 'selected' : '') + '" title="' + name + '" style="background-image:url('+iconsGroup[i].tab_url+');background-size: 20px;background-repeat: no-repeat;background-position: center;"></li>';
            }
            emoji_content += '</div>';
            ...
            $('#emoji_container_' + ix).css({'bottom': '0px', 'left': '0px'});
            ...
            function keyboardShowHandler(e){
               $(".message-detail-content")[0].style.height='auto';
            }
            
            
            function keyboardHideHandler(e){
                var index = $(".emoji_container").length-1;
                
                if($("#"+$(".emoji_container")[index].id).css("display")=='block')
                {
                    if(ionic.Platform.isIOS())
                    {
                        $(".message-detail-content")[0].style.height=($(document).height()-203)+'px';
                    }else
                    {
                        $(".message-detail-content")[0].style.height=($(document).height()-183)+'px';
                    }
                    ...
                    if (target === $(btn)[0]) {
                        ...
                        if (code) {
                            if (field.nodeName === 'DIV') {
                                ...
                                insertHtml = '<img style="max-width: 20px;max-height: 20px;" class="emoji_icon" src="' + imgSrc + '"/>';
        ...
        _insertAtCursor: function (field, value, selectPastedContent) {
            ...
            if (field.nodeName === 'DIV') {
                ...
                if (window.getSelection) {
                    ...
                    range.selectAllChildren(field);//range 选择obj下所有子内容
                    ...
                    range.moveToElementText(field);//range定位到obj
